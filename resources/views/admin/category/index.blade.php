...
@section('content')
    ...
    <button type="button" class="btn btn-secondary btn-lg active">
        @if(isset($category))
            <a href="{{url('admin/category/add/'.$category->id)}}">{{__('Thêm Danh Mục')}}</a>
        @else
            <a href="{{url('admin/category/add/1')}}">{{__('Thêm Danh Mục')}}</a>
        @endif
    </button>
                ...
                <form action="{{$url}}" method="POST" role="form text-left">
                    <div class="card-body pt-4 p-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <label for="user-name" class="form-control-label">{{ __('Đường dẫn') }}</label>
                                <div class="border rounded-3">
                                    @if(isset($category))
                                        <input class="form-control" value="{{$category->url}}" type="text"
                                               placeholder="url"
                                               id="attribute-label" name="url">
                                    @else
                                        <input class="form-control" value="" type="text" placeholder="url"
                                               id="attribute-label" name="url">
                                    @endif
                                </div>
                            </div>
